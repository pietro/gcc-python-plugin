...
class ConfigBuilder:
    ...
    def capture_shell_output(self, initmsg, cmd):
        with CheckFor(initmsg, mandatory=True):
            out = check_output(cmd, shell=True)  # input must be trusted
            out = str(out.decode())
        sys.stdout.write("%s\n" % out.strip())
        return out
    ...
    def test_for_mandatory_c_header(self, header, extraargs):
        with CheckFor(
            ...
            self.compile(test, src="#include <%s>" % header, extraargs=extraargs)

    def test_c_compilation(self, initmsg, src, extraargs, description, defn):
        with CheckFor(initmsg, okmsg="yes", failmsg="no", mandatory=False) as test:
            self.compile(test, src, extraargs)
        self.results.append(OptionFlag(description, test.succeeded(), defn))
