...
class Directive:
    ...
    def parse_args(args):
        ...
        ws = "\s+"
        m = re.match(
            quoted_group + ws + quoted_group + ws + "{(.*)}" + ws + "(.+)", args
        )
        if m:
...
class Result:
    ...
    def __str__(self):
        ...
        if self.directive:
            result += "%s:%i: " % (self.directive.inputfile, self.directive.linenum)
        if self.message:
            result += self.message
        return result

    def __repr__(self):
        return "Result(%r, %r, %r)" % (self.status, self.directive, self.message)
...
class DgContext:
    ...
    def handle_directive(self, directive):
        ...
        if directive.name == "dg-message":
            ...
            self.on_fail(directive, "unrecognized directive: %s" % directive.name)
    ...
    def end_multiline_output(self, directive):
        ...
        mr = ExpectedMultilineOutput(self._cur_multiline_output, start, end, lines)
...
class Tests(unittest.TestCase):
    def test_parse_line(self):
        ...
        d = ctxt.parse_line(
            INPUT_FILE, 42, 'before /* { dg-something "arg1" "arg2" } */ after'
        )
    ...
    def test_nonempty_stdout(self):
        ...
        self.assertEqual(ctxt.results[0].message, "non-empty stdout: 'stray text'")
    ...
    def test_surplus_errors(self):
        ...
        self.assertEqual(ctxt.results[0].status, "FAIL")
        self.assertEqual(
            ctxt.results[0].message, "unexpected output on stderr: 'stray text'"
        )
        self.assertEqual(ctxt.num_failures(), 1)
    ...
    def test_dg_message_found(self):
        ...
        ctxt = DgContext([INPUT_FILE])
        d = ctxt.parse_line(
            INPUT_FILE, 23, 'before /* { dg-message "17: hello world" } */ after'
        )
    ...
    def test_dg_options(self):
        ...
        ctxt = DgContext([INPUT_FILE])
        d = ctxt.parse_line(
            INPUT_FILE, 23, '/* { dg-options "-fdiagnostics-show-caret" } */'
        )
        ctxt.handle_directive(d)
    ...
    def test_dg_do(self):
        INPUT_FILE = "foo.c"
        ctxt = DgContext([INPUT_FILE])
        d = ctxt.parse_line(INPUT_FILE, 23, "/* { dg-do compile } */")
        ctxt.handle_directive(d)
