...
from cpybuilder import CompilationUnit, PyGetSetDefTable, camel_case, PyMethodTable
...
def generate_pass():
    ...
    getsettable = PyGetSetDefTable(
        "PyGccPass_getset_table",
        [],
        identifier_prefix="PyGccPass",
        typename="PyGccPass",
    )
    getsettable.add_simple_getter(
        cu, "name", "PyGccString_FromString(self->pass->name)", "Name of the pass"
    )
    getsettable.add_simple_getter(
        cu,
        "next",
    ...
    for field in ("properties_required", "properties_provided", "properties_destroyed"):
        getsettable.add_simple_getter(
            cu, field, "PyGccInt_FromLong(self->pass->%s)" % field, None
        )
    getsettable.add_simple_getter(
        cu,
        "static_pass_number",
...
def generate_pass_subclasses():
    ...
    for opt_pass_type in ("GIMPLE_PASS", "RTL_PASS", "SIMPLE_IPA_PASS", "IPA_PASS"):
