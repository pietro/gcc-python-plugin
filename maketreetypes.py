...
class GimpleStructType(
    namedtuple("GimpleStructType", "enum_value struct_name has_tree_operands")
):
    def camel_cased_string(self):
...
def iter_gimple_struct_types():
    ...
    for line in f:
        ...
        if m:
            ...
            #    print 'UNMATCHED: ', line
            assert (
                line.startswith("#") or line.strip() == "" or line.startswith("GIMPLE_")
            )
    f.close()
...
def iter_rtl_expr_types():
    ...
    for line in f:
        ...
        if m:
            # print m.groups()
            yield DefRtlExpr(
                ENUM=m.group(1), NAME=m.group(2), FORMAT=m.group(3), CLASS=m.group(4)
            )
        else:
