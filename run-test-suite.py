...
class TestStream:
    def __init__(self, exppath, srcdir):
        ...
        if os.path.exists(exppath):
            ...
            expdata = expdata.replace("PyStringObject", "PyBytesObject")
            expdata = expdata.replace("PyString_Type", "PyBytes_Type")
    ...
    def _cleanup(self, text):
        ...
        for line in text.splitlines():
            ...
            line = re.sub(" object at (0x[0-9a-f]*)>", " object at 0xdeadbeef>", line)
            ...
            line = re.sub(r"frozenset\(\[(.*)\]\)", r"frozenset({\1})", line)
            ...
            line = re.sub(r"input.c:([0-9]+):([0-9]+):", r"input.c:\1:nn:", line)
            line = re.sub(
                r"hash_format_codes.inc:([0-9]+):([0-9]+):",
            ...
            line = re.sub(
                r"r->ob_refcnt: \(long int\)val", r"r->ob_refcnt: (Py_ssize_t)val", line
            )
            ...
            line = re.sub(
                r"Ran ([0-9]+ tests?) in ([0-9]+\.[0-9]+s)", r"Ran \1 in #s", line
            )
    ...
    def diff(self, label):
        ...
        for line in unified_diff(
            self._cleanup(self.expdata).splitlines(),
        ...
        if cp.has_option("WhenToRun", "required_features"):
            ...
            for feature in required_features:
                if feature not in features:
                    raise ValueError(
                        "%s in %s not found in %s" % (feature, metadatapath, config_h)
                    )
                if not features[feature]:
                    raise SkipTest(
                        "required feature %s not available in %s" % (feature, config_h)
                    )
...
parser.add_option(
    ...
    type="string",
    dest="excluded_dirs",
    help="exclude tests in DIR and below",
    metavar="DIR",
)
parser.add_option("--srcdir", type="string", dest="srcdir", help="FIXME", metavar="DIR")
parser.add_option(
    "-s",
    "--show",
    action="store_true",
...
class TestRunner:
    ...
    def print_results(self):
        ...
        print(
            "%s; %s; %s"
            % (
                num(self.num_passes, "success", "successes"),
