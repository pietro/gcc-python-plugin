...
def get_gcc_version():
    p = Popen(["./print-gcc-version"], stdout=PIPE, stderr=PIPE)
...
class PyArg_ParseTupleTests(AnalyzerTests):
    def test_bogus_format_string(self):
        ...
        experr = (
            "$(SRCFILE): In function 'bogus_format_string':\n"
...
"""
    ...
    def get_expected_error(self):
        return (
            "$(SRCFILE): In function '%(function_name)s':\n"
    ...
    def test_format_code_K(self):
        self._test_format_code(
            "K", "unsigned PY_LONG_LONG", '"long long unsigned int *"'
        )
    ...
    def test_mismatched_parentheses(self):
        for code in ["(", ")", "(()"]:
            ...
            src += (
                '    if (!PyArg_ParseTuple(args, "%(code)s")) {\n'
                "              return NULL;\n"
                "    }\n"
                "    Py_RETURN_NONE;\n"
                "}\n"
            ...
            experr = (
                "$(SRCFILE): In function '%(function_name)s':\n"
                '$(SRCFILE):12:26: error: mismatched parentheses in format string "%(code)s"'
                % locals()
            )
            bm = self.assertFindsError(src, experr)
